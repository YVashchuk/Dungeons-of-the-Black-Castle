...
// Usage: OPENAI_API_KEY=sk-... node scripts/tts_pregenerate.js --lang ru [--voice onyx] [--model gpt-4o-mini-tts] [--from 1] [--to 1221] [--concurrency 3] [--dry]
...
const outDir = path.join(ROOT, 'dist', 'audio', LANG); fs.mkdirSync(outDir, { recursive: true });
...
(async () => { let i = 0, done = 0, failed = 0; const worker = async () => { while (i < todo.length) { const job = todo[i++]; try { await render(job); done++; if (done % 10 === 0) { fs.writeFileSync(manPath, JSON.stringify(manifest)); console.log('  ' + done + '/' + todo.length); } } catch (e) { failed++; console.error('  FAIL ' + e.message); } } }; await Promise.all(Array.from({ length: Math.max(1, CONC) }, worker)); for (const j of jobs) if (fs.existsSync(path.join(outDir, j.key + '.mp3'))) manifest[LANG][j.key] = true; fs.writeFileSync(manPath, JSON.stringify(manifest)); console.log('done: ' + done + ' rendered, ' + failed + ' failed, manifest ' + Object.keys(manifest[LANG]).length + ' entries -> ' + manPath); })();
