...
// and drop a protective _BATTERY_README so this never happens again. Run:  node tests/regen_fixtures.js
const fs=require('fs');
const path=require('path');
const REPO=path.resolve(__dirname,'..');
const A=path.join(__dirname,'goldens');
...
const L=JSON.parse(fs.readFileSync(path.join(REPO,'src','locale.ru.js'),'utf8').match(/const\s+LOCALE_RU\s*=\s*(\{[\s\S]*\})\s*;\s*$/)[1]);
...
fs.writeFileSync(path.join(A,'_6b_orig.json'),JSON.stringify(orig),'utf8');
// ---- 6d ----
const rfl={};
for(const n in L.p){ if(L.p[n].rfl!==undefined) rfl[n]=L.p[n].rfl; }
fs.writeFileSync(path.join(A,'_6d_data.json'),JSON.stringify({enemies:L.enemies,rfl:rfl}),'utf8');
// ---- 6e1 ----
fs.writeFileSync(path.join(A,'_6e1_map.json'),JSON.stringify(L.map),'utf8');
// ---- 6e2 ----
const keys={};
for(const m of mapSrc.matchAll(/\bt\('([a-z0-9_]+)'\)/g)){ const k=m[1]; if(L.ui[k]!==undefined) keys[k]=L.ui[k]; }
fs.writeFileSync(path.join(A,'_6e2_newkeys.json'),JSON.stringify(keys),'utf8');
