...
(async () => {
  ...
  while (steps < MAX_STEPS) {
    ...
    if (s.st <= 8 || (fightAhead && s.st <= s.stMax - 4)) { const h = await ev(() => { const sp = (S.spells || []).find(x => x.id === 'HEALING' && x.remaining > 0); if (sp && typeof useHealing === 'function') { useHealing(); return 'heal'; } if (S.flask > 0 && typeof useFlask === 'function') { useFlask(); return 'flask'; } return null; }); if (h) { L(h + ' at sec ' + s.sec + ' -> stamina ' + (await state()).st); continue; } }
    // choices
