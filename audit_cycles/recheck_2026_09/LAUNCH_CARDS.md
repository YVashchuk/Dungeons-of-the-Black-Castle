...
| 2026-09-05 | ChatGPT 6 Astra, агент с браузером, попытки №2 (тот же чат) и №3 (новый чат, архив 8b147bd) | прохождение | **ENV**: контроллер браузера падает на `browser.tabs.list()` до открытия страницы — инфраструктура ChatGPT; архивный пре-флайт PASS (v2.172, PA-01 подтверждён). Живая приёмка гейтов сделана пробником `tests/smoke/gate_probes.js` (29/29, `GATE_PROBES_2026-09-05_auto.md`); повторить агента, когда браузер ChatGPT заработает |
| 2026-09-05…09 | ChatGPT 6 Astra, агент с браузером + архив 8b147bd (Work Mode, проект) | прохождение + смоук | **принят**: первое полное внешнее прохождение (§1220 с обоими подвигами), G-01…G-15 14 PASS / 1 UNCLEAR, матрица 41 строки 29/2/7/3, 52 замечания → group_87 (38 закрыты в `437c7b2`/`449de1c`); отчёт и приложения — `audit_cycles/recheck_2026_09/astra_live/` |
| 2026-09-08 | Gemini (обычный чат, без пре-флайта, PDF диалога) | код/дизайн/мобильные | **не аудит** — советы: `build.sh` «сломан» и `mobile.css` «не подключён» — неверно (артефакты вставки, сборщик встраивает CSS); модалки «absolute» — неверно (fixed); карта «400+ узлов/force-layout» — неверно (16 узлов, фиксированные координаты); принято: `-webkit-tap-highlight-color` (сделано), SVG-иконки вместо эмодзи и TTS — в бэклог дизайна; `overscroll-behavior` и `will-change` — только после проверки на устройствах; PixiJS/Phaser — нет (DOM-ридер правильный выбор). Сгенерированная карта — художественный ориентир, не навигация (вымышленная география, «Barlcl Dert») |
| 2026-09-11 | Gemini Deep Research (Track C, только CSS/HTML/бриф) | стандарты | **принят частично**: 5 PASS, 3 улучшения сделаны (scroll-padding, `interactive-widget`, `overscroll-behavior`), 1 FAIL **опровергнут** (диапазоны весов стоят на вариативных шрифтах — проверено по таблицам woff2), 1 гипотеза про iOS оставлена до проверки на iPhone |
...
> **Статус 11.09.2026:** четыре попытки подряд сорвались в инфраструктуре провайдера (нативный confirm — исправлено PA-01; `tabs.list` ×2; transport disconnected). Карточку запускать только если браузер агента заработает; приёмка делается своими инструментами (group_96).
