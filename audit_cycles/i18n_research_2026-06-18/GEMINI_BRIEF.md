# Gemini — i18n / Localization Architecture Study (2026-06-18)
...
## ⚠ Use ONLY the files in THIS folder (context-safe extracts)
Your file reader silently truncates this project's real `remake_data.js` (one ~1 MB line) and the
base64 art files (`mj_art.js` 6.9 MB, `illustrations.js` 3.3 MB, `title_art.js` 370 KB). So you are
given extracts instead. Do not ask for the raw base64 — reason about images from the maps.

| File | What it is |
|---|---|
| `game_logic.js` | The FULL engine (~2530 lines) — every language-coupled code path is here. |
| `build.sh` + `build_shell.py` | How the single-file HTML is assembled. |
| `remake_data_SAMPLE.json` | 23 representative paragraphs (pretty-printed) showing the full `GD` schema, incl. items/riddles/food/enemies. The real data is 1221 such objects on one line. |
| `art_maps.txt` | paragraph→art-id maps (NO base64) + the art file sizes. |
| `language_coupling_inventory.md` | The precise list of every language-coupled surface (items, riddles, engine constants, UI strings). |
| `PROJECT_NOTES.md` | Architecture / engine-features overview. |
...
## Your emphasis (play to your strengths)
Lead on the **overall architecture and the player-facing UX**: the file/folder layout and locale schema;
the **language dropdown auto-detection** and the **instant state-preserving switch** (including mid-combat
/ modal cases); the per-language **text-bearing-image** strategy (regenerate vs web-font title); and a
clear recommendation on the `file://`-vs-served trade-off for the four target devices. Still produce the
FULL deliverable below.
