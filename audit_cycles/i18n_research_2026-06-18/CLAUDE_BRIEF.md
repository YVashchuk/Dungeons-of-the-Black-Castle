# Claude — i18n / Localization Architecture Study (2026-06-18)
...
Run as a NEW chat in the **Claude desktop app**, where `windows-mcp` (PowerShell + FileSystem) and the
`Filesystem` tools give direct access to the LOCAL project folder
`C:\Users\I828868\Downloads\Dungeons-of-the-Black-Castle`, and you can run **Python (`python -X utf8 s.py`)
/ node**. You do NOT need GitHub — the local files ARE the repo. (A claude.ai **web** session will NOT
work: the repo is private — no GitHub connector → 404 — and the web container has no disk.)
Tooling notes: never line-grep the single-line `remake_data.js` and never inline Cyrillic in
`python -c` from PowerShell (mojibake) — write a `.py` and run it with `python -X utf8`. Parse the data
with `GD = json.loads(re.match(r'\s*const\s+GD\s*=\s*(\{.*\})\s*;?\s*$', raw, re.S).group(1))`.

## Sources of truth
`src/game_logic.js` (engine) · `src/remake_data.js` (data) · `build.sh` + `scripts/build_shell.py` (build)
· `src/mj_art.js` / `src/illustrations.js` / `src/title_art.js` (base64 art + maps — inspect the MAPS,
not the blobs) · `PROJECT_NOTES.md` · `assets/text_corrections.json` (registry, v2.95 — context only).
...
## Your emphasis (broadest — you can run analysis in-project)
**Quantify and synthesize.** Reproduce/extend the coupling inventory with your own scripts (exact item
id list, every UI string site, every food-regex site, the 7 riddles), then produce the **synthesized
recommended architecture** + a **phased, low-risk migration plan** that keeps the Russian build playable
at every step, + the **per-platform validation matrix** with an explicit verdict on `file://`-vs-served.
Where you can, prototype-test a claim (e.g. does an inline per-language locale object avoid the `fetch`/
`file://` problem?) rather than asserting it. Produce the FULL deliverable below.
...
## If asked to proceed to implementation
Treat THIS pass as design only — do not edit code unless Yuriy explicitly green-lights a verified plan,
and always leave `git push` to Yuriy.
