...
function bcSpeakToggle(srcId){ srcId=srcId||'s-text'; if(_voiceSpeaking&&_voiceSrc===srcId){ bcStopSpeech(); return; } const ok=bcSpeak(bcSpeechText(srcId),srcId); if(!ok) bcNotice(t('ui_voice_unavailable')); }
function bcVoiceButton(){ document.querySelectorAll('.speak-btn').forEach(function(b){ const on=_voiceSpeaking&&(b.dataset.src||'s-text')===_voiceSrc; b.style.display=voiceSupported()?'':'none'; b.setAttribute('aria-pressed',on?'true':'false'); b.textContent=on?'\u23f9':'\ud83d\udd0a'; const lbl=on?t('ui_btn_speak_stop'):t('ui_btn_speak'); b.setAttribute('aria-label',lbl); b.title=lbl; }); }
function bcVoiceAutoRead(){ if(_voice.auto&&voiceSupported()){ setTimeout(function(){ try{ bcSpeak(bcSpeechText('s-text'),'s-text'); }catch(e){} },60); } }
