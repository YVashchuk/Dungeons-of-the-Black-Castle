...
// ── Screens ──
function showScr(id){ try{ bcStopSpeech(); }catch(e){} /* group_88 V-02: speech does not outlive its screen */document.querySelectorAll('.scr').forEach(s=>s.classList.remove('on'));document.getElementById('scr-'+id).classList.add('on');const elb=document.getElementById('event-log-btn');if(elb)elb.style.display=(id==='game'&&!isMobileHud())?'block':'none';}
...
let _voice={auto:false,rate:1,voiceURI:{},engine:'device',openaiKey:'',openaiVoice:'onyx',openaiModel:'gpt-4o-mini-tts'}; let _voiceQueue=[]; let _voiceSpeaking=false;
...
function loadVoicePrefs(){ try{ const v=JSON.parse(localStorage.getItem(VOICE_KEY)||'null'); if(v&&typeof v==='object'){ _voice.auto=!!v.auto; _voice.rate=Math.min(1.3,Math.max(0.7,Number(v.rate)||1)); _voice.voiceURI=(v.voiceURI&&typeof v.voiceURI==='object')?v.voiceURI:{}; _voice.engine=(v.engine==='openai')?'openai':'device'; _voice.openaiKey=typeof v.openaiKey==='string'?v.openaiKey:''; _voice.openaiVoice=typeof v.openaiVoice==='string'&&v.openaiVoice?v.openaiVoice:'onyx'; _voice.openaiModel=typeof v.openaiModel==='string'&&v.openaiModel?v.openaiModel:'gpt-4o-mini-tts'; } }catch(e){} }
...
let _voiceSrc='s-text';
function bcSpeechText(srcId){ const el=document.getElementById(srcId||'s-text'); if(!el) return ''; const clone=el.cloneNode(true); clone.querySelectorAll('img,figure,figcaption,.illust,.art-block').forEach(n=>n.remove()); return String(clone.textContent||'').replace(/\(\s*\d{1,4}\s*\)/g,'').replace(/\s+/g,' ').trim(); }
function bcSplitSentences(txt,max){ max=max||220; const parts=String(txt).match(/[^.!?\u2026]+[.!?\u2026]+["\u00bb)]?|[^.!?\u2026]+$/g)||[String(txt)]; const out=[]; let buf=''; for(const p of parts){ if((buf+p).length>max&&buf.trim()){ out.push(buf.trim()); buf=p; } else buf+=p; } if(buf.trim()) out.push(buf.trim()); return out; }
function bcSpeak(text,srcId){ if(!text) return false; bcStopSpeech(); _voiceSrc=srcId||'s-text'; _voiceSpeaking=true; bcVoiceButton(); const token=++_ttsToken; bcSpeakSmart(text,_voiceSrc,token); return true; }
function bcSpeakDevice(text,srcId){ if(!voiceSupported()||!text) return false; try{ speechSynthesis.cancel(); }catch(e){} _voiceQueue=[]; _voiceSrc=srcId||'s-text'; const v=bcPickVoice(); const lang=BC_LANG_TAGS[getLang()]||'ru-RU'; _voiceQueue=bcSplitSentences(text).map(c=>{ const u=new SpeechSynthesisUtterance(c); u.lang=(v&&v.lang)||lang; if(v) u.voice=v; u.rate=_voice.rate; u.pitch=1; return u; }); _voiceSpeaking=true; bcSpeakNext(); bcVoiceButton(); return true; }
...
function bcStopSpeech(){ _voiceQueue=[]; _voiceSpeaking=false; _voiceLoading=false; _ttsToken++; try{ if(_ttsAudio){ _ttsAudio.pause(); _ttsAudio.removeAttribute('src'); _ttsAudio.load(); } }catch(e){} try{ if(voiceSupported()) speechSynthesis.cancel(); }catch(e){} bcVoiceButton(); }
function bcSpeakToggle(srcId){ srcId=srcId||'s-text'; if((_voiceSpeaking||_voiceLoading)&&_voiceSrc===srcId){ bcStopSpeech(); return; } const ok=bcSpeak(bcSpeechText(srcId),srcId); if(!ok) bcNotice(t('ui_voice_unavailable')); }
function bcVoiceButton(){ document.querySelectorAll('.speak-btn').forEach(function(b){ const mine=(b.dataset.src||'s-text')===_voiceSrc; const on=_voiceSpeaking&&mine; b.style.display=''; b.setAttribute('aria-pressed',on?'true':'false'); b.textContent=(_voiceLoading&&mine)?'\u23f3':(on?'\u23f9':'\ud83d\udd0a'); const lbl=(_voiceLoading&&mine)?t('ui_voice_loading'):(on?t('ui_btn_speak_stop'):t('ui_btn_speak')); b.setAttribute('aria-label',lbl); b.title=lbl; }); }
function bcVoiceAutoRead(){ if(_voice.auto){ setTimeout(function(){ try{ bcSpeak(bcSpeechText('s-text'),'s-text'); }catch(e){} },60); } }
function renderVoiceMenu(){ const row=document.getElementById('voice-row'); if(!row) return; row.style.display=''; const eng=document.getElementById('voice-engine'); if(eng) eng.value=_voice.engine; const oa=document.getElementById('voice-openai'); if(oa) oa.style.display=(_voice.engine==='openai')?'':'none'; const k=document.getElementById('voice-key'); if(k) k.value=_voice.openaiKey||''; const ov=document.getElementById('voice-openai-voice'); if(ov) ov.value=_voice.openaiVoice||'onyx'; const om=document.getElementById('voice-openai-model'); if(om) om.value=_voice.openaiModel||'gpt-4o-mini-tts'; const dv=document.getElementById('voice-device'); if(dv) dv.style.display=voiceSupported()?'':'none'; const auto=document.getElementById('voice-auto'); if(auto) auto.checked=_voice.auto; const rate=document.getElementById('voice-rate'); if(rate) rate.value=String(_voice.rate); const sel=document.getElementById('voice-select'); if(sel){ const list=bcVoicesFor(getLang()); sel.innerHTML=''; if(!list.length){ const o=document.createElement('option'); o.value=''; o.textContent=t('ui_voice_none'); sel.appendChild(o); sel.disabled=true; } else { sel.disabled=false; const cur=bcPickVoice(); list.forEach(v=>{ const o=document.createElement('option'); o.value=v.voiceURI; o.textContent=v.name+(v.localService===false?' \u2601':''); if(cur&&cur.voiceURI===v.voiceURI) o.selected=true; sel.appendChild(o); }); } } }
function voiceMenuChanged(){ const auto=document.getElementById('voice-auto'); const rate=document.getElementById('voice-rate'); const sel=document.getElementById('voice-select'); if(auto) _voice.auto=!!auto.checked; if(rate) _voice.rate=Math.min(1.3,Math.max(0.7,Number(rate.value)||1)); if(sel&&sel.value) _voice.voiceURI[getLang()]=sel.value; const eng=document.getElementById('voice-engine'); if(eng) _voice.engine=(eng.value==='openai')?'openai':'device'; const k=document.getElementById('voice-key'); if(k) _voice.openaiKey=String(k.value||'').trim(); const ov=document.getElementById('voice-openai-voice'); if(ov&&ov.value) _voice.openaiVoice=ov.value; const om=document.getElementById('voice-openai-model'); if(om&&om.value) _voice.openaiModel=om.value; saveVoicePrefs(); }
...
// --- V-03 (group_88): OpenAI narration - bring your own key, browser cache, optional pre-rendered files, device fallback ---
const TTS_CACHE='dbc-tts-v1';
let _ttsAudio=null, _ttsToken=0, _ttsManifest=null, _voiceLoading=false;
function bcVoiceEngine(){ return (_voice.engine==='openai'&&_voice.openaiKey)?'openai':'device'; }
function bcHash(s){ let h=5381; for(let i=0;i<s.length;i++) h=((h<<5)+h+s.charCodeAt(i))|0; return (h>>>0).toString(16); }
async function bcTtsManifest(){ if(_ttsManifest!==null) return _ttsManifest; try{ const r=await fetch('audio/manifest.json',{cache:'no-cache'}); _ttsManifest=r.ok?await r.json():false; }catch(e){ _ttsManifest=false; } return _ttsManifest; }
function bcPlayUrl(url,token){ return new Promise(function(res,rej){ if(token!==_ttsToken) return res(); if(!_ttsAudio){ _ttsAudio=new Audio(); _ttsAudio.preload='auto'; } const a=_ttsAudio; a.onended=function(){ res(); }; a.onerror=function(){ rej(new Error('audio')); }; a.src=url; a.playbackRate=1; a.play().catch(rej); }); }
async function bcTtsBlob(text,lang){ const voice=_voice.openaiVoice||'onyx', model=_voice.openaiModel||'gpt-4o-mini-tts'; const speed=Math.min(1.5,Math.max(0.7,Number(_voice.rate)||1)); const key='https://dbc.tts/'+lang+'/'+model+'/'+voice+'/'+speed+'/'+bcHash(text); let cache=null; try{ if(window.caches){ cache=await caches.open(TTS_CACHE); const hit=await cache.match(key); if(hit) return await hit.blob(); } }catch(e){} const body={model:model,voice:voice,input:text,response_format:'mp3',speed:speed}; if(model.indexOf('gpt-4o')===0) body.instructions=t('ui_voice_style'); const r=await fetch('https://api.openai.com/v1/audio/speech',{method:'POST',headers:{'Authorization':'Bearer '+_voice.openaiKey,'Content-Type':'application/json'},body:JSON.stringify(body)}); if(!r.ok) throw new Error('openai '+r.status); const blob=await r.blob(); try{ if(cache) await cache.put(key,new Response(blob,{headers:{'Content-Type':'audio/mpeg'}})); }catch(e){} return blob; }
async function bcSpeakOpenAI(text,srcId,token){ const lang=getLang(); const chunks=bcSplitSentences(text,3500); _voiceLoading=true; bcVoiceButton(); try{ for(const c of chunks){ if(token!==_ttsToken) return; const blob=await bcTtsBlob(c,lang); if(token!==_ttsToken) return; _voiceLoading=false; bcVoiceButton(); const url=URL.createObjectURL(blob); try{ await bcPlayUrl(url,token); } finally { URL.revokeObjectURL(url); } } }catch(e){ console.warn('tts',e); if(token===_ttsToken){ _voiceLoading=false; bcNotice(t('ui_voice_err')); if(voiceSupported()){ bcSpeakDevice(text,srcId); return; } } } if(token===_ttsToken){ _voiceLoading=false; _voiceSpeaking=false; bcVoiceButton(); } }
async function bcSpeakSmart(text,srcId,token){ const lang=getLang(); const key=(srcId==='s-text'&&typeof S!=='undefined'&&S)?String(S.section):String(srcId).replace('-text',''); try{ const man=await bcTtsManifest(); if(token!==_ttsToken) return; if(man&&man[lang]&&man[lang][key]){ await bcPlayUrl('audio/'+lang+'/'+key+'.mp3',token); if(token===_ttsToken){ _voiceSpeaking=false; bcVoiceButton(); } return; } }catch(e){} if(token!==_ttsToken) return; if(bcVoiceEngine()==='openai'){ return bcSpeakOpenAI(text,srcId,token); } if(voiceSupported()){ bcSpeakDevice(text,srcId); return; } _voiceSpeaking=false; bcVoiceButton(); bcNotice(t('ui_voice_unavailable')); }
// <<< BC_VOICE <<<
...
function renderDiceRoll(sec){
  ...
  list.innerHTML='';
  // group_85 AS-14: the rolled face is committed before it is shown and survives a reload until the player continues.
  const showResolved=(roll,tgt)=>{
    list.innerHTML='';
  ...
  const pend=S&&S.pendingDiceRoll;
  if(pend&&pend.section===S.section){ showResolved(pend.roll,pend.tgt); return; }
  const btn=document.createElement('button');
  btn.className='choice-btn';
  ...
  btn.onclick=()=>{
    ...
    S.pendingDiceRoll={section:S.section,roll:roll,tgt:tgt}; saveGame();
    logEvent('luck',t('kubik_vypalo')+roll, tgt?(t('paragraf_3')+tgt):'');
    const cont=showResolved(roll,tgt);
...
function applyBetting(sec){
  if(!S||!sec) return;
  const notifs=[];
  // group_85 AS-14: a ledger of betting rounds - a stake is charged once per pending round, a payout once per
  // round; same-paragraph reloads (or a hash re-entry) neither charge nor pay twice.
  S.betLedger=S.betLedger||{}; const rid=S.betRoundId||0; const key=String(sec.id);
  // 1) Stake commit
  if(sec.set_stake&&!(S.betLedger[key]===rid&&S.bet_stake)){
    S.betRoundId=rid+1; S.betLedger[key]=S.betRoundId;
  ...
  if(sec.bet_payout&&S.betLedger[key]!==(S.betRoundId||0)){
    S.betLedger[key]=(S.betRoundId||0);
